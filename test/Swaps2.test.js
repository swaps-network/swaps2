...
contract("Swaps2", ([owner, myWish, broker, orderOwner, ...accounts]) => {
  ...
  it("deposit to order with broker and mywish and check distribution", async () => {
    ...
    let tx = await swaps.createOrder(
      baseToken.address,
      quoteToken.address,
      baseLimit,
      quoteLimit,
    ...
    //console.log("tx.logs[0].args.id =", tx.logs[0].args.id);
    const id = tx.logs[0].args.id;
  ...
  it("deposit to order with mywish and check distribution", async () => {
    ...
    for (let i = 0; i < baseAmounts.length; i++) {
      const investorQuoteToReceive = baseAmounts[i]
        .mul(quoteLimit.sub(myWishQuoteToReceive))
        .div(baseLimit);
    ...
    for (let i = 0; i < quoteAmounts.length; i++) {
      const investorBaseToReceive = quoteAmounts[i]
        .mul(baseLimit.sub(myWishBaseToReceive))
        .div(quoteLimit);
